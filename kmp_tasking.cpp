...
// Specify the allocation type
#define ALLOC_TYPE -1
// -1: Basic scheduler
//  0: Heuristic-based scheduler, MNTP (minimum number of task-parts)
//  1: Heuristic-based scheduler, MTET (minimum total execution time)

// Specify the dispatching type
#define DSPCH_TYPE -1
// -1: Basic scheduler
//  0: Heuristic-based scheduler, MET (minimum execution time)
//  1: Heuristic-based scheduler, HET (highest execution time)

//Variables to enable static mapping/scheduling
#if ALLOC_TYPE == -1 // Basic scheduler
    bool disable_stealing = false;
#else // Heuristic-based scheduler
    bool disable_stealing = true;
#endif

bool enable_tdg_scheduling = true;
// Set it to false for the benchmarks without any data dependency
...
static kmp_int32 __kmp_push_task(kmp_int32 gtid, kmp_task_t *task) {
  ...
  // Perform the allocation process using the heuristic-based scheduler
  if (ALLOC_TYPE != -1) {
    kmp_int32 sel_tid; // The selected thread

    // The MNTP heuristic
    if (ALLOC_TYPE == 0) {
      // Assume that Thread 0 contains the minimum number of tasks
      sel_tid = 0;
      thread_data = &task_team->tt.tt_threads_data[0];
      if (UNLIKELY(thread_data->td.td_deque == NULL)) {
        __kmp_alloc_task_deque(thread, thread_data);
      }
      kmp_int32 min_ntasks = thread_data->td.td_deque_ntasks; // The minimum number of tasks

      // Look for the remaining threads to find the one including the least tasks
      kmp_int32 ntasks;
      for (kmp_int32 i = 1; i < task_team->tt.tt_nproc; i++) {
        thread_data = &task_team->tt.tt_threads_data[i];
        if (UNLIKELY(thread_data->td.td_deque == NULL)) {
          __kmp_alloc_task_deque(thread, thread_data);
        }

        ntasks = thread_data->td.td_deque_ntasks; // The number of tasks for Thread i
    ...
    // The MTET heuristic
      ...
      // Assume that Thread 0 contains the minimum total execution time of tasks
      sel_tid = 0;
      thread_data = &task_team->tt.tt_threads_data[0];
      if (UNLIKELY(thread_data->td.td_deque == NULL)) {
        __kmp_alloc_task_deque(thread, thread_data);
      }
      kmp_int64 min_tot_exe_time = thread_data->td.tot_exe_time; // The minimum total execution time

      // Look for the remaining threads to find the one including the least time
      kmp_int64 tot_exe_time;
      for (kmp_int32 i = 1; i < task_team->tt.tt_nproc; i++) {
        thread_data = &task_team->tt.tt_threads_data[i];
        if (UNLIKELY(thread_data->td.td_deque == NULL)) {
          __kmp_alloc_task_deque(thread, thread_data);
        ...
        tot_exe_time = thread_data->td.tot_exe_time; // The total execution time of tasks for Thread i
        if (tot_exe_time < min_tot_exe_time) {
          sel_tid = i;
          min_tot_exe_time = tot_exe_time;
        }
      }
    }

    tid = sel_tid; // Set tid by the selected thread id
    gtid = __kmp_gtid_from_tid(tid, __kmp_team_from_gtid(gtid)); // Change gtid based on the new tid
  ...
  // Update total execution time of tasks in deque
  if (ALLOC_TYPE != -1) {
    kmp_node_info *recordMap = taskdata->tdg->recordMap;
    // Look for the task into the recordMap
    for (kmp_int32 i = 0; i < taskdata->tdg->numTasks; i++) {
      if (task == recordMap[i].task) {
        thread_data->td.tot_exe_time += recordMap[i].exe_time;
      }
    }
  }
...
void __kmpc_thread_sched(kmp_int32 period, kmp_int32 deadline, kmp_int32 runtime) {
    struct sched_attr attr = {
            .size = sizeof (attr),
            .sched_policy = SCHED_DEADLINE,
            .sched_runtime = (uint64_t) runtime,
            .sched_deadline = (uint64_t) deadline,
            .sched_period = (uint64_t) period
    };  
    if (sched_setattr(0, &attr, 0) < 0)
      printf("Error on setting thread scheduling period:%d deadline:%d runtime:%d - %s\n", period, deadline, runtime, strerror(errno));
}
...
static kmp_task_t *__kmp_remove_my_task(kmp_info_t *thread, kmp_int32 gtid,
  ...
  // Perform the dispatching process using the heuristic-based scheduler
  if (thread_data->td.td_deque_ntasks >= 2) { // If there are at least two tasks in the queue
    if (DSPCH_TYPE != -1) {
      kmp_task_t *curr_task; // Current task
      kmp_node_info *recordMap; // Array of TDG nodes
      kmp_int32 sel_tid; // The selected task

      // The MET heuristic
      if (DSPCH_TYPE == 0) {
        unsigned long exe_time[thread_data->td.td_deque_ntasks]; // Execution time of the tasks

        // Determine execution time of each task
        for (kmp_int32 i = 0; i < thread_data->td.td_deque_ntasks; i++) {
          curr_task = KMP_TASKDATA_TO_TASK(thread_data->td.td_deque[i]);
          recordMap = thread_data->td.td_deque[i]->tdg->recordMap;

          // Look for the current task into the recordMap
          for (kmp_int32 j = 0; j < thread_data->td.td_deque[i]->tdg->numTasks; j++) {
            if (curr_task == recordMap[j].task) {
              exe_time[i] = recordMap[j].exe_time;
            }
          }
        }

        // Select the task with the minimum execution time
        sel_tid = 0;
        unsigned long min_exe_time = exe_time[0]; // The minimum execution time
        for (kmp_int32 i = 1; i < thread_data->td.td_deque_ntasks; i++) {
          if (exe_time[i] < min_exe_time) {
            sel_tid = i;
            min_exe_time = exe_time[i];
          }
        }
      }
      // The HET heuristic
      else if (DSPCH_TYPE == 1) {
        unsigned long exe_time[thread_data->td.td_deque_ntasks]; // Execution time of the tasks
        ...
        // Select the task with the highest execution time
        sel_tid = 0;
        unsigned long high_exe_time = exe_time[0]; // The highest execution time
        for (kmp_int32 i = 1; i < thread_data->td.td_deque_ntasks; i++) {
          if (exe_time[i] > high_exe_time) {
            sel_tid = i;
            high_exe_time = exe_time[i];
          }
        }
      }

      // Swap the selected task and the tail task
      kmp_taskdata_t *sel_taskdata = thread_data->td.td_deque[sel_tid];
      thread_data->td.td_deque[sel_tid] = thread_data->td.td_deque[tail];
      thread_data->td.td_deque[tail] = sel_taskdata;
    }
  }
  ...
  if (DSPCH_TYPE != -1) {
    kmp_node_info *recordMap = taskdata->tdg->recordMap;
    // Look for the (removed) task into the recordMap
    for (kmp_int32 i = 0; i < taskdata->tdg->numTasks; i++) {
      if (KMP_TASKDATA_TO_TASK(thread_data->td.td_deque[tail]) == recordMap[i].task) {
        thread_data->td.tot_exe_time -= recordMap[i].exe_time;
      }
    }
  }
